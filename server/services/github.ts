        ...
        const projects = FeaturedProjects.filter((project) => project.feature)
            .map((featured) => {
                const repo = repositories.find(
                    (repository) =>
                        normalizeRepoName(repository.name) === normalizeRepoName(featured.repo)
                );
...
export async function getProject(lookup: string): Promise<PortfolioProject | null> {
    ...
    return (
        projects.find((p) => normalizeRepoName(p.displayName) === normalizeRepoName(lookup)) ?? null
    );
}
